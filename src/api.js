...
const _flattenResponseData = arr =>
  arr.reduce((flattened, { data }) => [...flattened, ...data], []);
...
module.exports = {
  ...
  getMergeRequests: projects => {
    ...
    return Promise.all(requests).then(responses =>
      _flattenResponseData(responses)
    );
  },
  getParticipants: mergeRequests => {
    const promises = mergeRequests.map(mergeRequest =>
      _getProjectParticipants(mergeRequest)
    );
