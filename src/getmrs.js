...
try {
  ...
  if (err.code === "ENOENT") {
    ...
    console.log(
      slightlyDarker(
        'You can overwrite this config later by running "getmrs -c"\n'
      )
    );
...
async function run(config) {
  ...
  const selectedProjectNamesOrIds =
    args.p || process.env.GITLAB_PROJECTS || config.projects;
  const filteredProjects = selectedProjectNamesOrIds
